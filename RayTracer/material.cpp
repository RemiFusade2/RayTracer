...
void Material::initFromDOMElement(const QDomElement& e)
{
	...
	setRefractionIndex(e.attribute("refraction_index", "1.0").toFloat());
			...
			if (e.tagName() == "DiffuseColor")
				...
				if (e.tagName() == "SpecularColor")
				{
					...
					if (e.tagName() == "ReflectiveColor")
						...
						{
							setBumpMapScale(e.attribute("scale", "1.0").toFloat());
							if (e.hasAttribute("scaleU"))
								setBumpMapScaleU(e.attribute("scaleU", "1.0").toFloat());
							if (e.hasAttribute("scaleV"))
								setBumpMapScaleV(e.attribute("scaleV", "1.0").toFloat());
							loadBumpMapFromFile(e.attribute("file"));
						}
						else
...
void Material::loadBumpMapFromFile(const QString& fileName)
{
	if (fileName.isNull())
		QMessageBox::warning(NULL, "Material bump map error", "Material error : no bump map file name provided");
	else
		if (!bump_map_.load(fileName))
			QMessageBox::warning(NULL, "Material bump map error", "Unable to load Material bbump map from "+fileName);
}
...
Color Material::perturbation(float u, float v) const
{
	if (bumpMap().isNull())
		return Color(0,0,0);
	
	// Take bumpMap scaling into account
	u /= bumpMapScaleU();
	v /= bumpMapScaleV();
	
	// Compute pixel coordinates
	const int i = int(fabs(u - floor(u)) * bumpMap().width());
	const int j = int(fabs(v - floor(v)) * bumpMap().height());
	
	// Compute color
	return Color(bumpMap().pixel(i,j));
}
